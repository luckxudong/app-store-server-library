...
import {
    ...
    readBytes, getSignedPayloadVerifier
} from "../util";
...
const appleCerts = [
    'tests/resources/certs/AppleRootCA-G2.pem',
    'tests/resources/certs/AppleRootCA-G3.pem',
    'tests/resources/certs/AppleIncRootCertificate.pem',
    'tests/resources/certs/AppleComputerRootCertificate.pem',
];
...
describe("Decoding checks", () => {
    it('should fail to verify with a missing x5c header', async () => {
        ...
        try {
            ...
            expect(e).toBeInstanceOf(VerificationException)
            let status = (e as VerificationException).status;
            expect(status).toEqual(VerificationStatus.INVALID_CHAIN_LENGTH)
        }
    })
    ...
    it('PieceNotificationTest should verify and decode a valid notification', async () => {
        const verifier = getSignedPayloadVerifier(Environment.SANDBOX, 'com.the-next-world.piece', 6738498698, appleCerts, true)
        const testNotification = readFile('tests/resources/mock_signed_data/testNotificationForPiece')
        const notification = await verifier.verifyAndDecodeNotification(testNotification)
        expect(notification.notificationType).toEqual("TEST")
    })
